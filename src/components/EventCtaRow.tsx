...
export default function EventCtaRow({
  ...
  return (
    <div data-testid="event-cta-row" className="flex items-center gap-2">
      {showReserve && pick.soldOut ? (
        <span
          aria-disabled="true"
          className="inline-flex h-11 min-w-0 flex-1 cursor-default items-center justify-center rounded-lg border border-culture-line bg-culture-cream px-3 text-sm font-medium text-culture-muted"
        >
          Sold out
        </span>
      ) : null}
      {showReserve && !pick.soldOut && pick.url ? (
        <a
          ...
          className="inline-flex h-11 min-w-0 flex-1 items-center justify-center rounded-lg bg-culture-terracotta px-3 text-sm font-semibold text-white hover:bg-culture-clay"
        >
          Réserver
        </a>
      ) : null}
